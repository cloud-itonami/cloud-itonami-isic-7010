(ns headoffice.operation
  "OperationActor -- one head-office operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (HeadOffice-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Group Oversight Governor
  ...
  One graph run = one head-office operation (intake -> advise ->
  ...
  decision to a human operator (head-office staff). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:actuation/finalize-allocation` ALWAYS reaches this node when the
  governor is clean -- see `headoffice.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [headoffice.headofficeadvisor :as headofficeadvisor]
...
(defn build
  ...
  `headoffice.store/Store`).
  opts:
    :advisor      -- a `headoffice.headofficeadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (headofficeadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; HeadOffice-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (headofficeadvisor/-advise advisor store request)]
            {:proposal p :audit [(headofficeadvisor/trace request p)]})))

      ;; Group Oversight Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
